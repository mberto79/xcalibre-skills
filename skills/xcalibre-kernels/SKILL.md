...
- NO anonymous closures/captured lambdas inside `@kernel` — GPU compile fails. The
  `xcal_foreach do` block is the sanctioned closure form.
- Any custom struct passed into a kernel needs `Adapt.@adapt_structure` and concrete field types.
- Loop pattern by face class: INTERNAL faces → cell-based loops (no atomics); BOUNDARY faces
  → face-based loops with `Atomix.@atomic`. Boundary faces are `1:n_bfaces` in `mesh.faces`;
  internal faces follow (`+n_bfaces` offset).
- `@synchronize` splits the kernel body — never place it inside divergent branches/loops;
  `@localmem` element types must be compile-time (type param or `Val`, never runtime `eltype`).
- Kernels are distribution-oblivious: ghost cells are ordinary cells, never MPI inside a
  kernel; halo `sync!` belongs at solver seams, not in kernels.
