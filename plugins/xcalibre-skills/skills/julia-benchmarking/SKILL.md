...
- For full simulation benchmarks, using `@time run!(...)` for wall time is acceptable after a one-iteration `run!` warmup that compiles the solver path. Rebuild the configuration object with the target iteration count after warmup and reinitialise all fields with `initialise!` before measuring.
...
- Validate correctness after performance changes with the focused test and the full workflow when feasible.
