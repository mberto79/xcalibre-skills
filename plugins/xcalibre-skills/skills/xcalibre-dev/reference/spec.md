...
This file records for people what the product promises; it is not an instruction set for agents. Name the BINDING CORE - the few clauses that define the product and that no session relaxes - in one line here; every other clause is AMENDABLE by a session holding better evidence, under its own ID, with a decision saying why. A candidate departing from an amendable clause or from `dev/architecture.md` is measured, never refused. No clause names a mechanism, owner, milestone, date or ruling: those live in the other records.
...
What was considered and is out of scope, so it is not re-proposed.
