...
WHAT THIS FEATURE MUST BE TRUE OF WHEN IT IS DELIVERED. This spec is the FEATURE'S, never the host project's: it states what the feature must do and hold, and it inherits the project's own requirements rather than restating them. Where the host has a vault, name the requirement IDs of `dev/spec.md` that constrain this work; where it does not, name the behaviour of the existing code the feature may not break.
...
The request in the user's own terms, in a few lines, before it is interpreted. Anything the requirements below add to it is an INTERPRETATION and says so.
...
Outcomes only. A requirement the user did not state and the codebase does not imply is an OPEN QUESTION below, never a requirement invented on their behalf. Nothing here is frozen: a clause is amended under its own ID with a decision when the work shows it should be.
...
What the host project already requires that this feature must not break, by ID where the project has them and by name where it does not.
...
Each with what it blocks and who or what settles it. A question whose answer changes the shape of the work is asked before that work starts, not after.
